...
# Download miraligner from seqbuster if not already present
if [ ! -f "$BASE_DIR/seqbuster-miraligner-3.5.zip" ]; then
...
for SAMPLE in $SAMPLES; do
    ...
    if [ -f "$TRIMMED_FILE" ]; then
        ...
        java -jar "$MIRALIGNERJAR" -sub 1 -trim 3 -add 3 -s mmu -i "$TRIMMED_FILE" -db "$BASE_DIR/seqbuster-miraligner-3.5/miraligner/DB" -o "$BASE_DIR/miraligner/$SAMPLE_NAME" -freq > "$BASE_DIR/miraligner/${SAMPLE_NAME}_log.txt" 2>&1
...
# Create a temporary directory for GFF outputs
TEMP_GFF_DIR="$MIRTOP/temp_gff"
...
for SAMPLE in $SAMPLES; do
    ...
    if [ -f "$MERGED_BAM_FILE" ]; then
        ...
        mirtop gff --hairpin "$HAIRPIN_FILE" --gtf "$MIRTOP/mmu.gff" -o "$TEMP_GFF_DIR/$SAMPLE_NAME.gff" --sps mmu "$MERGED_BAM_FILE"
    else
        echo "Merged BAM file not found for sample: $SAMPLE_NAME"
    fi
done

# Combine all individual GFF files into one
cat $TEMP_GFF_DIR/*.gff > "$MIRTOP/mirtop_combined.gff"

# Additional mirtop steps
mirtop counts --hairpin "$HAIRPIN_FILE" --gtf "$MIRTOP/mmu.gff" -o "$MIRTOP" --sps mmu --add-extra --gff "$MIRTOP/mirtop_combined.gff"
mirtop export --format isomir --hairpin "$HAIRPIN_FILE" --gtf "$MIRTOP/mmu.gff" --sps mmu -o "$MIRTOP" "$MIRTOP/mirtop_combined.gff"
mirtop stats "$MIRTOP/mirtop_combined.gff" --out "$MIRTOP/stats"
Rscript collapse_mirtop.r "$MIRTOP/mirtop_combined.gff"

# Run MultiQC to aggregate FastQC reports
multiqc "$BASE_DIR" -o "$BASE_DIR/multiqc"
